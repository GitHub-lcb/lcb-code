  ...
  <div class="container">
    <div class="lucky-wheel">
      ...
      <div class="wheel-main">
        <div class="wheel-pointer" :class="canClick===true ?'img-onlyRead' : ''" @click="beginRotate()"></div>
...
<!--        <div @click="HolidayFuli()" class="div_class_defaultBtn">特殊福利(点击领取)</div>-->
        ...
        <br/>
        <div class="count">剩余快乐币数量： {{ beginDraw.money }}</div>
        <br/>
        <div class="count">抽奖总次数： {{ beginDraw.totalCount }}</div>
      </div>

<!--      <div class="tip">-->
<!--        <div class="tip-title">活动规则</div>-->
<!--        <div class="tip-content">-->
<!--          <p>1.每日签到后，即可获得10次幸运大转盘的机会，仅限当天有效，过期作废。</p>-->
<!--          <p>2.快乐币抽奖，每50快乐币可兑换一次大转盘抽奖机会。</p>-->
<!--          <p>3.每次保底500钻石和随机5星英雄碎片5个</p>-->
<!--        </div>-->
<!--      </div>-->
    </div>
  ...
  methods: {
    ...
    initPrizeList() {
      ...
      getItemsAndPersonInfo().then(response => {
        this.beginDraw.id = response.data.id;
        this.beginDraw.money = response.data.money;
        this.beginDraw.roleName = response.data.roleName;
        this.beginDraw.totalCount = response.data.totalCount;
        this.count = response.data.count;
        const prizeList = [] = response.data.items;
    ...
    async beginRotate() {
      ...
      if (this.beginDraw.money < 50 && this.count === 0) {
        return
      }
      // 设置不让重复点击
      this.canClick = true;
      ...
      // 随机获取下标
      const res = await getReward(this.beginDraw);
      console.log(res)
      this.index = res.data.num;
      if (this.count > 0){
        this.count--
      }
      this.beginDraw.money = res.data.money
    ...
    rotateOver() {
      this.isRotating = false
      this.prize = this.prizeList[this.index-1]
